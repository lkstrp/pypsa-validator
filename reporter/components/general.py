...
from reporter.meta.config import PLOTS, RESULTS_SUBFOLDER
...
def plots_table() -> str:
    ...
    for plot in plots_list:
        if not plot:
            continue
        url_a = URL_IMAGE_PLACEHOLDER.format(f"main/results/{RESULTS_SUBFOLDER}/{plot}")
        url_b = URL_IMAGE_PLACEHOLDER.format(f"feat/results/{RESULTS_SUBFOLDER}/{plot}")
        rows.append(
            [
